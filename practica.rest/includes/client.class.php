<?php
    ...
    class Client{
        ...
        public static function update_client($id, $email, $name, $city, $telephone){
            ...
            $state = $conn->prepare('UPDATE listado_clientes SET email=:email, name=:name, city=:city,telephone=:telephone WHERE id=:id');
            $stmt->bindParam(':email',$email);
            $stmt->bindParam(':name',$name);
            $stmt->bindParam(':city',$city);
            $stmt->bindParam(':telephone',$telephone);
            ...
            if($stmt->execute()){
                ...
                header('HTTP/1.1 404 Cliente no se ha podido actulizar correctamente');    
            }
        }
